...
class DatePickerItem extends Component<void, Props, State> {
  ...
  componentDidMount() {
    ...
    this.container = document.getElementById("react-mobile-datepicker-20");
    ...
    viewport.addEventListener("wheel", throttle(this.handleWheel, 70), false);
  }
  ...
  componentWillUnmount() {
    ...
    viewport.removeEventListener("wheel", this.handleWheel, false);
  ...
  handleWheel(event) {
    ...
    if (Math.abs(event.deltaY * 4) < 12) return;
    const date = this.state.dates[MIDDLE_INDEX];
    const { max, min } = this.props;
    if (date.getTime() < min.getTime() || date.getTime() > max.getTime()) {
      return;
